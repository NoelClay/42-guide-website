...
이제 이 하드웨어 설계를 바탕으로 VirtualBox 설정을 진행하십시오. 가상 머신은 단순한 소프트웨어가 아니라, **엄밀하게 계산된 가상 하드웨어 시스템**입니다. [하드웨어 세팅 과정](how_to_set_hardware_detailed_report.md)

---

## Part 2 - Debian 설치 화면 따라가기 (학부 수준 보충)

하드웨어 세팅이 끝나면 실제 Debian 설치 화면(VM/사용자/파티셔닝/소프트웨어)을 진행하게 됩니다. 그 단계에서 **"내가 뭘 하고 왜 이걸 골랐는지"** 를 평가자에게 답할 수 있을 정도로 정리한 보충 가이드는 별도 문서로 분리되어 있습니다.

→ [Part 2 — Debian 설치 화면 따라가기](./install_part2_os_install_guide.md)

깊이는 학부 전공 필수 수준으로 묶었습니다 (박사/논문급 폭주 없음). 부팅 흐름의 더 깊은 배경은 [`MBR_완역_주석_0장.md`](./_reference/MBR_완역_주석_0장.md) 를 참고하십시오.
