...
본 보고서는 운영 체제와 애플리케이션 서비스가 실제로 점유하는 자원을 정밀하게 산출하여, 안정적인 서버 운영을 위한 하드웨어 사양을 확정하는 과정을 기술합니다. 왜 우리가 이 수치를 선택해야만 했는지, 운영 체제와 하이퍼바이저의 스케줄링 원리를 통해 그 타당성을 순차적으로 증명해 나가겠습니다. 리눅스 환경에 익숙하지 않은 독자라도 각 서비스의 메모리 구조와 CPU 스케줄링 원리를 통해 이 수치의 타당성을 충분히 이해할 수 있을 것입니다.
...
본 하드웨어 설계 과정은 단순히 숫자를 채우는 과정이 아니라, 서버가 구동될 생태계의 물리적 한계를 인지하고 병목을 제거해 나가는 고민의 흔적입니다. 어떤 결정을 내리든, 그 근거를 스스로 밝힐 수 있어야 그것이 진짜 지식이 됩니다.

---

## 6. 참고 문헌 및 인용 근거 (References)
